...
function fixWordArtifacts(text) {
  ...
  // Remove track changes marks (e.g., [Deleted: text], [Inserted: text])
  fixed = fixed.replace(/\[(?:deleted|inserted|moved|formatted|comment)[^\]]*\]/gi, '');

  // Remove comments (e.g., {comment: text})
  fixed = fixed.replace(/\{(?:comment|note)[^}]*\}/gi, '');
...
function fixOCRArtifacts(text) {
  ...
  // Fix broken option markers from OCR (only at line start, not inside words)
  fixed = fixed.replace(/^([A-D])\s*\.\s*/gm, '$1) '); // A. -> A) at line start
  fixed = fixed.replace(/^([A-D])\s*\-\s*/gm, '$1) '); // A- -> A) at line start
...
function fixMessagingArtifacts(text) {
  ...
  // Remove WhatsApp-specific patterns (e.g., [10:20 AM, 5/1/2024] timestamps)
  fixed = fixed.replace(/\[\d{1,2}:\d{2}\s*(?:AM|PM)[^\]]*\]/gi, '');
