...
export function classifyQuestion(questionData) {
  ...
  // NOTE: Only return types supported by the Question model:
  // mcq, essay, true_false, fill_blank, short_answer

  // MCQ: has options and short question - check FIRST since options are the strongest signal
  if (hasABCOptions && options.length >= 2 && !hasRubric) {
    return QUESTION_TYPES.MCQ;
  }

  // Diagram questions: explicit diagram keywords -> map to essay
  if (diagramKeywords.test(lowerText)) {
    return QUESTION_TYPES.ESSAY;
  }

  // Practical questions: explicit practical keywords -> map to essay
  if (practicalKeywords.test(lowerText)) {
    return QUESTION_TYPES.ESSAY;
  }

  // Matching questions: explicit matching keywords + typically has options -> map to mcq
  if (matchingKeywords.test(lowerText) && hasABCOptions) {
    return QUESTION_TYPES.MCQ;
  }

  // Calculation questions: explicit calculation keywords -> map to essay
  ...
  // Essay: has marking scheme, or question starts with essay verbs (discuss, explain, analyze, etc.)
  // Even short essay questions starting with imperative verbs should be essay
  // Strip question number prefix (e.g., "Question 2", "1.", "2)", "Question 2 ") before checking verbs
  const strippedText = lowerText.replace(/^(?:question\s+)?\d+[.):\s]\s*/i, '').trim();
  const startsWithEssayVerb = /^(discuss|explain|analyze|evaluate|describe|calculate|find|determine|prove|derive|write|outline|account|state|list|name)/i.test(strippedText);
  
  if (hasRubric || (essayKeywords.test(lowerText) && (isLongQuestion || startsWithEssayVerb))) {
    return QUESTION_TYPES.ESSAY;
  }
...
export function classifyQuestions(questions) {
  return questions.map(q => ({
    ...
    type: classifyQuestion({
      text: q.questionText || '',
      options: q.options || [],
      // Only treat as having a marking scheme if it was actually extracted from text,
      // not if it's an auto-generated default rubric
      hasMarkingScheme: !!(q.markingScheme && 
        q.markingScheme.length > 0 && 
        !q.markingScheme.startsWith('Marking Scheme (Total:')),
    }),
  }));
}
