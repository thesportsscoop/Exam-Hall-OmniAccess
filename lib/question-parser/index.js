...
export async function parseQuestions(text) {
  ...
  try {
    ...
    for (const section of sections) {
      ...
      // Skip unknown sections (headers/instructions) if we have real question sections
      if (section.type === 'unknown' && hasQuestionSections) continue;
      ...
      for (const boundary of mergedBoundaries) {
        ...
        // Get correct answer from answer map (only for MCQ questions)
    ...
    if (allQuestions.length === 0) {
      ...
      for (const boundary of fallbackBoundaries) {
        ...
        result.questions.push({
          ...
          type: questionType,
          options,
          correctAnswer: questionType === 'mcq' ? (answerMap[boundary.questionNumber.toString()] || '') : '',
          markingScheme: rubric || (questionType === 'essay' ? generateDefaultRubric(questionText, marks) : ''),
          points: marks,
