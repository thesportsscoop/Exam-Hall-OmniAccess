  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const boundary of mergedBoundaries) {
        ...
        const correctAnswer = answerMap[boundary.questionNumber.toString()] || '';
    ...
    if (allQuestions.length === 0) {
      ...
      for (const boundary of fallbackBoundaries) {
        ...
        result.questions.push({
          questionText,
          type: classifyQuestion({ text: questionText, options, hasMarkingScheme: rubric.length > 0 }),
          options,
          correctAnswer: answerMap[boundary.questionNumber.toString()] || '',
          markingScheme: rubric || generateDefaultRubric(questionText, marks),
          points: marks,
