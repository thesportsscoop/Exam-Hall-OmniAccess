...
export function validateQuestion(question, index = 0) {
  ...
  // Validate question type (only types supported by the Question model)
  const validTypes = ['mcq', 'essay', 'true_false', 'fill_blank', 'short_answer'];
  if (!question.type || !validTypes.includes(question.type)) {
    warnings.push(`Question ${qNum}: Invalid or missing question type '${question.type}', defaulting to 'essay'`);
    question.type = 'essay';
    confidence -= 10;
  }
...
function validateMCQ(question, qNum) {
  ...
  if (!question.correctAnswer || question.correctAnswer.trim().length === 0) {
    warnings.push(`Question ${qNum}: No correct answer specified - please select one`);
    confidencePenalty += 10;
