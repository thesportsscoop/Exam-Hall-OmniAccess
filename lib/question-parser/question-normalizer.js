...
function normalizeAnswer(answer, options) {
  if (!answer || answer.trim().length === 0) {
    // Leave answer empty if not provided - teacher will fill in
    return '';
  }
  ...
  if (options && options.length > 0) {
    ...
    if (!validLabels.includes(normalized)) {
      // Return first option if answer is invalid, otherwise empty
...
export function repairOCRText(text, confidence = 0.7) {
  ...
  for (const fix of ocrFixes) {
    // Apply fixes deterministically - the confidence field is informational
