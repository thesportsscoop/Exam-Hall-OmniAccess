...
const GEMINI_API_KEYS = [
  process.env.GEMINI_API_KEY || 'GEMINI_API_KEY_FROM_ENV',
  process.env.GEMINI_API_KEY_BACKUP || 'GEMINI_API_KEY_FROM_ENV',
].filter(Boolean);
...
export async function validateWithAI(questions, originalText) {
  ...
  try {
    ...
    // AI validation is optional - don't fail the request
  }
...
async function runAIValidation(questions, originalText) {
  // Build a summary of parsed questions
...
Be brief and specific. Only flag actual issues.`;
  ...
  for (let attempt = 0; attempt < GEMINI_API_KEYS.length; attempt++) {
    ...
    try {
      ...
      // Parse AI response into warnings
      const warnings = [];
      const suggestions = [];
