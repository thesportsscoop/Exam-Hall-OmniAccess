...
export function extractOptions(lines, startIndex) {
  ...
  while (i < lines.length) {
    ...
    // Stop at "Answer: X" lines - they mark the end of the question block
    if (
      /^(?:the\s+)?(?:correct\s+)?(?:answer|ans|key)\s*[:=\-–—]/i.test(line) ||
      /^(?:the\s+)?(?:correct\s+)?answer\s+is\s+/i.test(line)
    ) {
      break;
    }
    
    // Match option patterns: A) text, B. text, C - text, D: text, (A) text
    // Only uppercase A-D are MCQ options; lowercase a-d are essay sub-questions
    ...
    if (optionMatch) {
      ...
      // We've already found options, and this isn't an option - stop
      break;
    } else if (line.length > 0) {
      // Not an option line yet - skip ahead (could be question text)
      i++;
    } else {
      i++;
    }
...
export function mergeSplitOptions(lines, startIndex) {
  ...
  while (i < lines.length) {
    ...
    // Stop at "Answer: X" lines
    if (
      /^(?:the\s+)?(?:correct\s+)?(?:answer|ans|key)\s*[:=\-–—]/i.test(line) ||
      /^(?:the\s+)?(?:correct\s+)?answer\s+is\s+/i.test(line)
    ) {
      // Save current option before breaking
      if (currentOption) {
...
export function recoverSplitOptions(text) {
  ...
  // Remove answer key lines first
  text = text.replace(/(?:the\s+)?(?:correct\s+)?(?:answer|ans|key)\s*[:=\-–—]\s*\(?\s*[A-Ea-e]\s*\)?\.?/gi, '');
  ...
  if (options.length === 0) {
    ...
    while ((match = numberedPattern.exec(text)) !== null) {
      ...
      // 1->A, 2->B, etc. Clamp to E (5) max
      const label = String.fromCharCode(64 + Math.min(num, 5));
      options.push({
        label,
        text: match[2].trim(),
      });
