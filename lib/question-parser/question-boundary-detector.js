...
function detectByNumbering(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numberMatch && numberMatch[2].length > 5) {
 ...
 */
export function mergeBoundaries(boundaries, maxGap = 2) {
  ...
  for (let i = 1; i < boundaries.length; i++) {
    ...
    // If next question starts within maxGap lines of current ending, merge
    if (next.startLine <= current.endLine + maxGap) {
