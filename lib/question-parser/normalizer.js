...
export function normalizeText(text) {
  ...
  // Step 8: Remove excess horizontal whitespace within lines (NOT newlines)
  normalized = normalized.replace(/[ \t]+/g, ' ');

  // Step 9: Ensure option markers are on their own lines
  // Match A) B) C) D) that might be on same line as question
  normalized = normalized.replace(/([^\n])(\s*[A-Da-d]\s*[)\].\s\-:]\s+)/g, '$1\n$2');

  // Step 10: Final cleanup - remove leading/trailing whitespace from each line
